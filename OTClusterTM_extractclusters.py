...
if __name__ == "__main__":
    ...
    cluster_emb = np.load(os.path.join(
        dir, 'cluster_embeddings.npy')).reshape(-1, 384)
    ckpt = torch.load(os.path.join(dir, 'checkpoint.pt'))
    model.load_state_dict(ckpt)
    topic_prj = model.topic_emb_prj(torch.from_numpy(
        topic_embeddings).to('cuda')).detach().cpu().numpy()

    topic_cluster_dist = torch.cdist(torch.from_numpy(
        topic_prj), torch.from_numpy(cluster_emb))
    cluster_id_of_topic = topic_cluster_dist.argmin(axis=1)

    topic_words_10 = static_utils.get_topic_words(beta, dataset.vocab, 10)
    # pprint.pprint(topic_words_10)
    group_list = [[], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []]
    group_id_list = [[], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []]
    for i in range(100):
        group_list[cluster_id_of_topic[i].item()] += [topic_words_10[i]]
        group_id_list[cluster_id_of_topic[i].item()].append(i)

    print(group_list[torch.tensor(18)])

    for i in range(20):
        print('cluster: ', i)
        for topic, words in zip(group_id_list[i], group_list[i]):
            print(topic, ':', words)
