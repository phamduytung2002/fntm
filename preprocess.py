...
def preprocess_data(train_infile, test_infile, output_dir, train_prefix, test_prefix, min_doc_count=0, max_doc_freq=1.0, vocab_size=None, stopwords=None, keep_num=False, keep_alphanum=False, strip_html=False, lower=True, min_length=3, label_fields=None):
    ...
    if stopwords == 'mallet':
        ...
        stopword_list = fh.read_text(os.path.join('data', 'stopwords', 'mallet_stopwords.txt'))
    elif stopwords == 'snowball':
        print("Using snowball stopwords")
        stopword_list = fh.read_text(os.path.join('data', 'stopwords', 'snowball_stopwords.txt'))
    elif stopwords is not None:
        print("Using custom stopwords")
        stopword_list = fh.read_text(os.path.join('data', 'stopwords', stopwords + '_stopwords.txt'))
    else:
        stopword_list = []
    ...
    fh.write_list_to_text(list(vocab), os.path.join(output_dir, 'vocab.txt'))
...
def process_subset(items, parsed, label_fields, label_lists, vocab, output_dir, output_prefix):
    ...
    for i, words in enumerate(parsed):
        ...
        if len(counter.keys()) > 0:
            # udpate the counts
            mallet_strings.append(' '.join(word_subset))
    ...
    # print(sparse_X[0])
    fh.save_sparse(sparse_X, os.path.join(output_dir, output_prefix + '_bow.npz'))
    ...
    fh.write_list_to_text(mallet_strings, os.path.join(output_dir, output_prefix + '_texts.txt'))
    ...
    if len(dat_labels) > 0:
        fh.write_list_to_text(dat_labels, os.path.join(output_dir, output_prefix + '_labels.txt'))

    # save output for Jacob Eisenstein's SAGE code:
    #sparse_X_sage = sparse.csr_matrix(X, dtype=float)
    vocab_for_sage = np.zeros((vocab_size,), dtype=np.object_)
